...
def createItems(windows, spans=None):
    ...
    for win in windows:
        ...
        if icon.strip() == '' and len(win.wm_class.replace('org.gnome.', '').split('.')) > 1:
            ...
            print(remove_prefix(win.wm_class, 'org.gnome.').split('.')[1].lower())    
            icon = getIcon(remove_prefix(win.wm_class, 'org.gnome.').split('.')[1].lower())    
        print("Icon: "+icon+win.wm_class)
