...
local file = home .. "/.config/conky/resources/xp_green.png"
function conky_bat_bar(str)
    local percent = 0
    percent = tonumber(conky_parse(str))
    local cs = cairo_xlib_surface_create(conky_window.display, conky_window.drawable, conky_window.visual, conky_window.width, conky_window.height)
    local cr = cairo_create(cs)
    --#########################################################################################################
    --local imtab=loadstring("return" .. imtab)()
    local im_h = 0
    local im_w = percent*3.64
    local show = imlib_load_image(file)
    if show == nil then return "fail" end
    ...
    imlib_context_set_image(show)
    local scaled=imlib_create_cropped_scaled_image(0, 0, width, imlib_image_get_height(), width, height)
    --local scaled=imlib_create_cropped_scaled_image(0, 0, 50, imlib_image_get_height(), 50, height)
    imlib_free_image()
    imlib_context_set_image(scaled)
    imlib_render_image_on_drawable(0, 16)
    imlib_free_image()
    show=nil
    --#########################################################################################################
    imtab=nil
...
function conky_bat_warn(str)
    local b_status = conky_parse('${eval $${exec acpi | grep "Battery 0:" | cut -d " " -f3}}')
	local val = string.gsub(string.sub(conky_parse(str),3,8),'%%','')
	local percent = tonumber(val)

    if b_status == "Charging," then
        if prev_lvl < 99 then
		    os.execute('notify-send "Charging" -u low -i battery-100-charging -h string:x-canonical-private-synchronous:battery -t 1')
            os.execute('paplay ~/.config/conky/resources/orb.ogg')
        end
        prev_lvl = 101
    elseif b_status == "Full," then
        if prev_lvl < 102 then
            os.execute('paplay ~/.config/conky/resources/levelup.ogg')
        end
        prev_lvl = 102
    elseif prev_lvl ~= 99 then
        if percent <= 10 and prev_lvl > 10 then
    		os.execute('notify-send "Battery below 10%" -u critical -i battery-010 -h string:x-canonical-private-synchronous:battery -t 0')
            os.execute('~/.config/conky/resources/hurt.sh &')
    	elseif percent <= 15 and prev_lvl > 15 then
    		-- alert the user, put your command here
            os.execute('notify-send "Battery below 15%" -u normal -i battery-020 -h string:x-canonical-private-synchronous:battery -t 20000')
            os.execute('~/.config/conky/resources/hurt.sh &')
    	elseif percent <= 20 and prev_lvl > 20 then
    		-- alert the user, put your command here
            os.execute('notify-send "Battery below 20%" -u normal -i battery-020 -h string:x-canonical-private-synchronous:battery -t 20000')
            os.execute('~/.config/conky/resources/hurt.sh &')
        elseif percent <= 5 then
            --os.execute('paplay ~/.config/conky/resources/drown$((1+ RANDOM %4)).ogg &')
            os.execute('~/.config/conky/resources/hurt.sh &')
        elseif prev_lvl >100 then
            os.execute('paplay ~/.config/conky/resources/remove_item4.ogg')
        end
        prev_lvl = percent
	end
