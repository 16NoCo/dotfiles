...
Add dmenu formatting options and default terminal if desired to
    /!\ modified! /!\ 
~/.config/rofi/scripts/networkmenu-config.ini

old: ~/.config/networkmanager-dmenu/config.ini
...
font = expanduser("~/.local/share/fonts/MinecraftRegular-Bmg3.otf")
...
def dmenu_cmd(num_lines, prompt="/networks ", active_lines=None):  # pylint: disable=too-many-branches
    ...
    conf.read(expanduser("~/.config/rofi/scripts/networkmenu-config.ini"))
...
def choose_adapter(client):
    ...
    command = dmenu_cmd(len(devices), "/networks choose-adapter ")
    prompt = command[command.index('-p')+1]

    length = int(Popen([expanduser("~/.bin/text_width.py"), prompt, "20", font], stdout=PIPE)
            .communicate()[0].decode(ENC))
    max_length = int(Popen([expanduser("~/.bin/text_width.py"), '-max', device_names, "20", font], stdout=PIPE)
            .communicate()[0].decode(ENC))
    del(command[command.index('-location'):command.index('-location')+2])
    command.extend(['-theme-str', 'listview{margin:0 calc(100%- '+str(4*2+2*2+2+length+max_length)+'px) 0 '
            +str(length+2)+'px;}'])
...
def get_selection(eths, aps, vpns, gsms, blues, wwan, others):
    ...
    length = int(Popen([expanduser("~/.bin/text_width.py"), prompt, "20", font], stdout=PIPE)
            .communicate()[0].decode(ENC))
    max_length = int(Popen([expanduser("~/.bin/text_width.py"), '-max', inp_bytes, "20", font], stdout=PIPE)
            .communicate()[0].decode(ENC))
    del(command[command.index('-location'):command.index('-location')+2])
    command.extend(['-theme-str', 'listview{margin:0 calc(100%- '+str(4*2+2*2+2+length+max_length)+'px) 0 '
            +str(length+2)+'px;}'])
...
def get_passphrase():
    ...
    if pinentry:
        ...
        command = dmenu_cmd(0, "/networks password ")
        prompt = command[command.index('-p')+1]
...
def delete_connection():
    ...
    command = dmenu_cmd(len(conn_acts), "/networks delete ")
    prompt = command[command.index('-p')+1]

    length = int(Popen([expanduser("~/.bin/text_width.py"), prompt, "20", font], stdout=PIPE)
            .communicate()[0].decode(ENC))
    max_length = int(Popen([expanduser("~/.bin/text_width.py"), '-max', conn_names, "20", font], stdout=PIPE)
            .communicate()[0].decode(ENC))
    del(command[command.index('-location'):command.index('-location')+2])
    command.extend(['-theme-str', 'listview{margin:0 calc(100%- '+str(4*2+2*2+2+length+max_length)+'px) 0 '
            +str(length+2)+'px;}'])
